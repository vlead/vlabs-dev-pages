...
The virtual labs are built by a large community of developers
...
Bombay]].  Please contact them or email (TODO: provide vlead
email or a VLEAD website).
...
A lab is of two types:
  ...
  1. *Container*:  it merely holds together already existing
     experiments and some wrapper content like an
...
** What is Hosting?
Hosting is a service that takes the sources of a an
...
** What the virtual labs hosting service provides
   - 1. Infrastructure :: The application and its
        ...
        about finding their own cloud infrastructure.
...
** Hosting terminology
   ...
   - 9. Owner    :: The person or team taking responsibility
                    ...
                    virtual labs Hosting Team will using
                    this username to communicate with the
                    owner.  An owner must first register
...
build process, it will marking the hosting request as
*Failed* and alerting the Owner.
...
** Identity of the application
   An application is identified by its registration issue id on
...
** Source repository (Repo) of an Application
   An application's source code resides in a repository.  The
   mapping of an application to its repository may be changed.
...
** Hosting request
A hosting request is necessary before a registered application can
be hosted.  
...
** Labels and Status of a Hosting Request
    ...
    *Hosted*, *Reverted* or *Failed* issues may be closed.
...
The information in an update request consists of the the
...
* Deregistering an Application
   ...
   could perhaps the owner is no longer maintaining the
...
application may be suspended if it is found that it the
...
|----------------+-----------------------+------------------+-----------+---+---|
| Hosting        |                       |                  |           |   |   |
|                | Application  Id       | #number          | mandatory |   |   |
